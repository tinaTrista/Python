...
#
    ...
    def ldap_get_vaild(self,uid=None,passwd=None):
        ...
        try:
            if obj.simple_bind_s(target_cn, self.hash_md5(password)):
                return True
            else:
                return False
        except ldap.LDAPError,e:
    ...
    def ldap_update_pass(self,uid=None,oldpass=None,newpass=None):
        modify_entry = [(ldap.MOD_REPLACE,'userpassword',self.hash_md5(newpass))]
        obj = self.ldapconn
        target_cn = self.ldap_search_dn(uid)
        try:
            obj.simple_bind_s(target_cn,self.hash_md5(oldpass)）
            obj.passwd_s(target_cn, self.hash_md5(oldpass）,self.hash_md5(newpass)）
            return True
        except ldap.LDAPError,e:
            return False
    # md5加密
